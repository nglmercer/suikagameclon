...
const resizeCanvas = () => {
    ...
    let newWidth = screenWidth; // Ajustar el ancho al tamaño de la ventana
    let newHeight = screenHeight; // Ajustar el alto al tamaño de la ventana
    let scaleUI = 1;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Agregar los elementos generados al contenedor
    menuStatics.forEach(static => {
        const staticElement = document.createElement('div');
        staticElement.style.position = 'absolute';
        staticElement.style.left = `${static.position.x}px`;
        staticElement.style.top = `${static.position.y}px`;

        const sprite = static.render.sprite;
        if (sprite) {
            const spriteImg = document.createElement('img');
            spriteImg.src = sprite.texture;
            spriteImg.style.width = `${sprite.xScale * newWidth}px`; // Ajustar el ancho de acuerdo al nuevo tamaño
            spriteImg.style.height = `${sprite.yScale * newHeight}px`; // Ajustar el alto de acuerdo al nuevo tamaño
            staticElement.appendChild(spriteImg);
        }

        staticElement.style.pointerEvents = 'none'; // Para evitar la interacción con los elementos generados

        staticElement.style.zIndex = '-1'; // Establecer un índice Z para que estén en el fondo
    });
});